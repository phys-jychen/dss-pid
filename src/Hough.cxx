...
Hough::Hough(const vector<Double_t>& x, const vector<Double_t>& y, const vector<Double_t>& z, const vector<Double_t>& e)
{
...
//    cout << "Initialising..." << endl;
    ...
    for (Int_t i = 0; i < nlayer; ++i)
        layer_hits[i] = vector<Hough::hit*>();

    for (Int_t j = 0; j < x.size(); ++j)
    {
        Int_t layer = z.at(j) / thick;

        Hough::hit* tmp_hit = new Hough::hit();
    ...
    for (Int_t i = 0; i < nlayer; ++i)
    {
        vector<vector<Hough::hit*>> tmpcol = this->MergeAdjacent(layer_hits[i]);
        ...
        for (auto t : tmpcol)
        {
            ...
            {
                Hough::hcl* tmp_hcl = new Hough::hcl();
    ...
    map<pair<Int_t, Int_t>, Int_t> map_track;
...
vector<vector<Hough::hit*>> Hough::MergeAdjacent(vector<Hough::hit*> hits)
{
    vector<vector<Hough::hit*>> vechit;

    auto fadj = [] (const Hough::hit* _hit, const vector<Hough::hit*> _hits)->Bool_t
    {
    ...
    while (hits.size() > 0)
    {
        vechit.emplace_back(vector<Hough::hit*>({hits.at(0)}));
